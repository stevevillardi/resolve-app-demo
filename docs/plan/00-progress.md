...
| 2 | Design system | [02-design-system.md](02-design-system.md) | Done | Theme tokens (signal-rail accent system, light/dark), 11 more shadcn/Base UI components installed, full static/mock-data app shell — all 16 components from blueprint §10 (+ GitHubConnectDialog, see decision below) built and reachable. All 6 acceptance checks verified via `npm run dev` + lint/typecheck/build, in both light and dark mode. |
