...
## 1. Migration `0004`, additive only

`drizzle/0004_add_model_columns.sql`, generated by `drizzle-kit generate` after
editing `src/main/db/schema.ts`. Additive-only per the Phase 4 decision that
splits additive and destructive migrations (drizzle-kit's rename-vs-drop prompt
needs a TTY, so a mixed migration fails outright in a non-interactive shell).

| Table | Column | Why |
|---|---|---|
| `persona_templates` | `model TEXT NULL` | scope item 6; null = the backend's default, so no backfill |
| `usage_events` | `model TEXT NULL` | the acceptance check "attributed to the model that actually ran" has no column today |
| `usage_events` | `cost_source TEXT NULL` | `'sdk'` vs `'computed'`; Phase 10 must label estimates honestly |

`AgentUsage` also carries `cacheWriteInputTokens` and `reasoningOutputTokens`.
Deliberately **not** added — neither appears in blueprint §12 and neither feeds
the cost formula. Note the omission in the migration comment so Phase 10 doesn't
rediscover it.

Mirror in `src/shared/domain.ts`: `personaTemplateSchema.model`
(`z.string().nullable()`), and `usageEventSchema.model` / `.costSource`
(reusing `costSourceSchema` from `src/shared/agent.ts`). Update
`toPersonaTemplate` / `toUsageEvent` in `src/main/db/mappers.ts` — note its
`optional()` helper turns SQL NULL into an *omitted* key, and `model` on a
persona is explicitly nullable, so it follows the `backendSessionId` precedent,
not the `cachedInputTokens` one.

Tests for db-backed services run the **real checked-in migrations** via
`createTestDb()` in `src/main/db/test-db.ts`, so nothing needs hand-written DDL.

## 2. Per-persona model selection
...
Honesty note to put in the code comment, not just here: Codex `read_only` is
OS-enforced (`--sandbox read-only`), so "readers cannot write" is a guarantee
there. Claude's is `disallowedTools` plus the bash classifier in
`src/main/adapters/sandbox.ts`, and Phase 5 recorded that the SDK's own
classifier decides first and short-circuits some commands before our gate sees
them (`echo hello` and `pwd && ls` both ran without reaching `evaluateToolUse`).
A Claude reader is policy-safe, not kernel-safe.
...
- Mapping: `text_delta` appends; `text_message` replaces (the Codex path);
...
- **Widen `MessageBubbleError['kind']`** in `src/renderer/src/types/message.ts`
  ...
  titles. `AgentErrorKind` is otherwise a superset that maps across with no
  translation table.
...
- Sandbox enforcement is Phase 5 work, already tested and proven live on both
  backends. Scope item 3 is a *demonstration* in the running app against
  Journey 1's scripted scenario, not new enforcement code.
