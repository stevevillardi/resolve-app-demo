 ...
 *
 * Phase 8 removed `routines` — RoutineList, RoutineEditor and the command
 * palette all read real rows now, and a scheduler fires them.
 *
 * These three survive because the usage dashboard is Phase 10. Their ids match
 * the first-run seed in src/main/db/seed-data.ts, so those screens join against
 * real personas rather than rendering orphans.
 */
