        ...
        {events.length === 0 ? (
            ...
            <tbody>
              {byPersona.map((row) => (
                <tr key={row.persona.id} className="border-border/70 border-b last:border-0">
                  ...
                  <td className="py-2 text-right font-mono tabular-nums">{formatCost(row.cost)}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <>
            <Section
              ...
              description="Stacked by persona. Tokens rather than dollars, so both backends are represented."
            >
