...
export interface ActiveRun {
  ...
  groupId: string | null
  /**
   * The write this turn is paused on, when it is paused on one. On the run row
   * rather than its own query so the approval card costs no new wiring:
   * `runs.list` already refetches on every `runs-changed`, and a pending ask
   * emits exactly that.
   */
...
function startTurn(
  ...
  try {
    ...
    const adapter = adapterForBackend(persona.backend, {
      ...
      // The ask_writes hold: the Claude adapter awaits this inside
